 ...
 if (attachButton && fileInput) {
    attachButton.addEventListener('click', (e) => {
        // Если заблокировано или нет сессии — блокируем действие
        if (isProcessingDocument || !sessionId) {
            e.preventDefault();
            return;
        }
                if (attachButton.contains(fileInput)) {
            // Ничего не делаем — нативный click по <label> активирует вложенный <input>
            return;
        }
        // Иначе — fallback: программно открываем picker
        e.preventDefault();
        try {
            fileInput.click();
        } catch (err) {
            console.error('Не удалось открыть диалог выбора файла программно:', err);
        }
    });
